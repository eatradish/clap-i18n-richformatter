...
#[non_exhaustive]
pub struct ClapI18nRichFormatter;

impl ErrorFormatter for ClapI18nRichFormatter {
...
fn translation_errorkind(error: &clap::error::Error<ClapI18nRichFormatter>) -> String {
    match error.kind() {
...
fn write_dynamic_context(
    error: &clap::error::Error<ClapI18nRichFormatter>,
...
fn did_you_mean(styled: &mut StyledStr, styles: &Styles, context: &str, possibles: &ContextValue) {
    ...
    if let ContextValue::String(possible) = possibles {
        ...
        if possibles.len() == 1 {
            let possible = format!("{valid}{}{valid:#}", &possibles[0]);
            let _ = write!(
                styled,
                " {}",
                fl!(
